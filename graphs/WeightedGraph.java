...
public class WeightedGraph<T> implements WeightedGraphInterface<T>
{
  ...
  //create new array marks[] initialized to false
  {
	  marks = new boolean[DEFCAP];
  }
  ...
  // get vertex index
  //check value and return if marked or not
  {
	  for(int i=0;i<numVertices;i++) {
		  if(vertices[i].equals(vertex)) {
			  if(marks[i] == true) {
  ...
  // search marks[] for unmarked vertex
  // return first found or null if none found
  {
	  for(int i=0;i<numVertices;i++) {
		  if(marks[i] == false) {
			  return vertices[i];
